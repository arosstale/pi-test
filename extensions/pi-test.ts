...
export default function (api: ExtensionAPI) {
	...
	// ── Showboat: "Show Your Work" Test Evidence Pattern ───────────
	// From Simon Willison's agentic manual testing pattern (Mar 2026):
	// "Never assume code generated by an LLM works until that code has been executed."
	// Agents should document what they tested, how, and what they observed.
	api.registerCommand({
		name: "showboat",
		description: "Generate a 'show your work' test evidence report for the current project",
		handler: async (args, ctx) => {
			const cwd = process.cwd();
			const { framework, cmd } = detectFramework(cwd);
			...
			const report: string[] = [];
			report.push(`# 🎪 Showboat — Test Evidence Report`);
			report.push(`> "Never assume code generated by an LLM works until that code has been executed."`);
			report.push(`> — Simon Willison, Agentic Manual Testing (Mar 2026)\n`);
			report.push(`**Project:** ${cwd}`);
			report.push(`**Date:** ${new Date().toISOString().slice(0, 19)}`);
			report.push(`**Framework:** ${framework}\n`);

			// 1. Run automated tests
			report.push(`## 1. Automated Tests`);
			if (framework !== "unknown" && cmd) {
				const { output, exitCode, duration } = runTests(cmd, cwd);
				const result = parseResults(output, framework, exitCode, duration);
				report.push(`- **Status:** ${result.success ? "✅ PASS" : "❌ FAIL"}`);
				report.push(`- **Results:** ${result.passed} passed, ${result.failed} failed, ${result.skipped} skipped (${result.total} total)`);
				report.push(`- **Duration:** ${(result.duration / 1000).toFixed(1)}s`);
				if (result.failed > 0) {
					report.push(`\n### Failure Output`);
					report.push("```");
					report.push(output.slice(-1500));
					report.push("```");
				}
			} else {
				report.push(`- ⚠️ No test framework detected`);
			}
			...
			try {
				const tscOut = execSync("npx tsc --noEmit 2>&1", { encoding: "utf-8", cwd, timeout: 60000 });
				staticChecks.push(`- ✅ TypeScript compilation: clean`);
			} catch (e: any) {
				const errors = ((e.stdout || "") + (e.stderr || "")).split("\n").filter((l: string) => l.includes("error TS"));
				staticChecks.push(`- ❌ TypeScript: ${errors.length} error(s)`);
				if (errors.length) {
					staticChecks.push("  ```");
					staticChecks.push("  " + errors.slice(0, 5).join("\n  "));
					staticChecks.push("  ```");
				}
			}
			try {
				const lintOut = execSync("npx eslint . --max-warnings=0 2>&1", { encoding: "utf-8", cwd, timeout: 60000 });
				staticChecks.push(`- ✅ ESLint: clean`);
			} catch (e: any) {
				const warnings = ((e.stdout || "") + (e.stderr || "")).match(/\d+ problems?/);
				staticChecks.push(`- ⚠️ ESLint: ${warnings ? warnings[0] : "issues found"}`);
			}
			report.push(staticChecks.length ? staticChecks.join("\n") : "- No static checks available");

			// 3. Manual smoke test guidance
			report.push(`\n## 3. Manual Smoke Tests (Agent Checklist)`);
			report.push(`> Agents: Execute these checks with \`node -e "..."\` or \`python -c "..."\` and record results\n`);
			report.push(`- [ ] Import/require main module without errors`);
			report.push(`- [ ] Call primary function with valid input → expected output`);
			report.push(`- [ ] Call primary function with edge case input → graceful handling`);
			report.push(`- [ ] Call primary function with invalid input → proper error`);
			report.push(`- [ ] Verify no unhandled promise rejections`);
			report.push(`- [ ] Verify no console.error/warn in normal flow`);

			// 4. Evidence footer
			report.push(`\n## 4. Evidence Chain`);
			report.push(`- **Generated by:** pi-test showboat`);
			report.push(`- **Timestamp:** ${new Date().toISOString()}`);
			report.push(`- **Working directory:** ${cwd}`);
			report.push(`\n---`);
			report.push(`*Pattern: Simon Willison's "Showboat" — agents show their work with documented test artifacts.*`);

			const reportText = report.join("\n");
			const reportPath = joinP(tmpD(), "showboat-report.md");
			writeF(reportPath, reportText);

			ctx.ui.notify(`${B}${CYAN}🎪 Showboat Report${RST}\n${reportText.slice(0, 800)}...\n\n${D}Full report: ${reportPath}${RST}`, "info");

			try {
				const { execSync: exec2 } = await import("node:child_process");
				exec2(`start "" "${reportPath}"`, { stdio: "ignore" });
			} catch {}
		},
	});
}
